...
for subscription in $(echo "${subscriptions[@]}" | jq -c '.[]'); do
    ...
    SP_OBJECT_ID=$(az ad sp list --display-name "acme"$(echo ${SUBSCRIPTION_NAME} | tr '[:upper:]' '[:lower:]' | sed -e 's/-//g') --query '[].{objectId:objectId}' -o tsv)
    echo "SP_OBJECT_ID is $SP_OBJECT_ID"

    DTS_GROUP_OBJECT_ID=$(az ad group list --display-name "DTS Platform Operations" --filter "displayname eq 'DTS Platform Operations' and securityEnabled eq "true"" --query '[].{objectId:objectId}' -o tsv)
    echo "DTS_GROUP_OBJECT_ID is $DTS_GROUP_OBJECT_ID"

    DTS_Public_DNS_Contributor_ID=$(az ad group show -g "DTS Public DNS Contributor (env:${ENVIRONMENT})" --query '{objectId:objectId}' -o tsv)
    ...
    APPLICATION_INSIGHT_ID=$(az monitor app-insights component show -a ${PRODUCT}-${ENVIRONMENT} -g ${PRODUCT}-${ENVIRONMENT}-rg --query '{id:id}' -o tsv)
        ...
        if [ "$1" = "--import" ]; then
           ...
           echo "Importing ACME resources into terraform state..."
            ...
            terraform import -var builtFrom=azure-enterprise -var env=${ENVIRONMENT} -var product=enterprise -var-file=../../environments/${ENVIRONMENT}/${ENVIRONMENT}.tfvars module.acme.azurerm_role_assignment.Reader ${FUNCTION_APP_READER}
            terraform import -var builtFrom=azure-enterprise -var env=${ENVIRONMENT} -var product=enterprise -var-file=../../environments/${ENVIRONMENT}/${ENVIRONMENT}.tfvars module.acme.azurerm_role_assignment.kvaccess ${FUNCTION_APP_KV_ACCESS}
            terraform import -var builtFrom=azure-enterprise -var env=${ENVIRONMENT} -var product=enterprise -var-file=../../environments/${ENVIRONMENT}/${ENVIRONMENT}.tfvars module.acme.azurerm_role_assignment.kvgroupaccess ${KV_GROUP_ACCESS}
            terraform import -var builtFrom=azure-enterprise -var env=${ENVIRONMENT} -var product=enterprise -var-file=../../environments/${ENVIRONMENT}/${ENVIRONMENT}.tfvars module.acme.azurerm_resource_group.rg ${RESOURCE_GROUP_ID}
            terraform import -var builtFrom=azure-enterprise -var env=${ENVIRONMENT} -var product=enterprise -var-file=../../environments/${ENVIRONMENT}/${ENVIRONMENT}.tfvars module.acme.azurerm_service_plan.asp ${APP_SERVICE_PLAN}
            terraform import -var builtFrom=azure-enterprise -var env=${ENVIRONMENT} -var product=enterprise -var-file=../../environments/${ENVIRONMENT}/${ENVIRONMENT}.tfvars module.acme.azurerm_windows_function_app.funcapp ${FUNCTION_APP_ID}
            terraform import -var builtFrom=azure-enterprise -var env=${ENVIRONMENT} -var product=enterprise -var-file=../../environments/${ENVIRONMENT}/${ENVIRONMENT}.tfvars module.acme.azuread_group_member.dnszonecontributor ${DTS_Public_DNS_Contributor_ID}/member/${FUNCTION_APP_OBJECT_ID}
            terraform import -var builtFrom=azure-enterprise -var env=${ENVIRONMENT} -var product=enterprise -var-file=../../environments/${ENVIRONMENT}/${ENVIRONMENT}.tfvars module.acme.azurerm_application_insights.appinsight ${APPLICATION_INSIGHT_ID}
